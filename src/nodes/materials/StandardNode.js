...
function StandardNode( isLegacy ) {

	this.isLegacy = isLegacy !== undefined ? isLegacy : false;

	GLNode.call( this );

	if (this.isLegacy) {
		
		this.surface = new ColorNode( 0xffffff ); // diffuse
		this.roughnessValue = new THREE.FloatNode( 1 );
		this.metalnessValue = new THREE.FloatNode( 1 );
		
		this.map = new THREE.TextureNode();
		
		this.normalMap = new THREE.TextureNode();
		this.normalMapScale = new THREE.Vector2Node( 1, 1 );
		
		this.roughnessMap = new THREE.TextureNode();
		
		this.metalnessMap = new THREE.TextureNode();
		
		this.envMap = new THREE.CubeTextureNode();
		...
		// shortcut: ignore build nodes and jump to buildCode
		
		this.build = this.buildCode;
...
StandardNode.prototype.build = function ( builder ) {
	...
	this.color = this.map.value ? new OperatorNode( this.surface, this.map, OperatorNode.MUL ) : this.surface;
	
	// slots: roughness, roughnessMap
	
	this.roughness = this.roughnessMap.value ? new OperatorNode( this.roughnessValue, this.roughnessMap, OperatorNode.MUL ) : this.roughnessValue;
	
	// slots: metalness, metalnessMap
	
	this.metalness = this.metalnessMap.value ? new OperatorNode( this.metalnessValue, this.metalnessMap, OperatorNode.MUL ) : this.metalnessValue;

	// slots: normalMap, normalScale
	
	this.normal = this.normalMap.value ? this.normalMap : undefined;
	this.normalScale = this.normalMapScale;
	...
	return this.buildCode( builder );
	
};

StandardNode.prototype.buildCode = function ( builder ) {
...
/*
	...
	if ( this.clearCoat || this.clearCoatRoughness ) material.define( 'PHYSICAL' );
*/
	material.define( 'STANDARD' );
