...
class Element extends Serializer {
	...
	connect( element = null ) {
		...
		if ( element !== null ) {
			...
			if ( dispatchEventList( this.events.valid, this, element ) === false ) {
		...
		for ( const link of links ) {
			...
			if ( draggingLink || length === 1 ) {
				...
				drawLine(
					...
					false, 2, colorA || '#ffffff', drawContext, colorB || null
				);
				...
				for ( let i = 0; i < length; i ++ ) {
					...
					const colorA = rioLength > 1 ? color : lioElement.getRIOColor() || '#ffffff';
					const colorB = lioLength > 1 ? color : rioElement.getLIOColor() || '#ffffff';
