...
import { scriptable, js, scriptableValue } from 'three/nodes';

const defaultTitle = 'Scriptable';
const defaultWidth = 500;

export class ScriptableEditor extends BaseNode {
	...
	constructor( source = null, enableEditor = true ) {
		...
		if ( source && source.isScriptableNode ) {

			codeNode = source.codeNode;
			scriptableNode = source;
	...
	update( force = false ) {
		...
		try {
			...
			scriptableValueOutput = this.scriptableNode.getOutput();
	...
	addElementFromJSON( json ) {
		...
		const { id, element, node } = createElementFromJSON( json );

		this.add( element );

		this.scriptableNode.setParameter( id, node );
