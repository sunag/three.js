...
import { createValueSpan, setText, getText } from '../ui/utils.js';
...
class Performance extends Tab {
	...
	resolveStats( inspector, stats ) {
		...
		if ( item === undefined ) {
			...
			setText( item.domElement.querySelector( '.list-item-cell' ), item.data[ 0 ] );
	...
	updateNotInUse( uid ) {
		...
		const remaining = 3 - Math.floor( ( current - time ) / 1000 );

		if ( remaining >= 0 ) {

			const counter = '*'.repeat( Math.max( 0, remaining ) );
			const element = item.domElement.querySelector( '.list-item-cell' );

			setText( element, item.data[ 0 ] + ' (not in use) ' + counter );
